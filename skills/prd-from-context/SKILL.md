...
5. Напиши PRD по шаблону ниже и сохрани markdown-файл в `docs/product/` (предпочтительно) или `docs/prd/`, если такая папка уже используется задачей.

   Не включай конкретные пути к файлам и фрагменты кода. Они быстро устаревают и не должны быть частью PRD.

   Для AI Collector дополнительно сверяй тон и термины с `PRODUCT_LANGUAGE.md` и compliance-first ограничениями продукта.
