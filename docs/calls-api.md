...
Маршрут намеренно fail-closed: при `LIVE_CALLS_ENABLED=false` ответ `403 LIVE_CALLS_DISABLED`; при включённом флаге — `501 LIVE_CALLS_NOT_IMPLEMENTED` (HTTP Exolve и end-to-end orchestrator ещё не подключены). Он не проксирует запрос в sandbox, не создаёт `CallAttempt` и не вызывает поставщика.
