...
- Формат: CSV (значения через запятую) или XLSX с тем же набором колонок.
- Кодировка CSV: UTF-8.
- Формулы Excel не вычисляются: берётся только сохранённое значение ячейки.
- Пустой лист XLSX → ошибка валидации `EMPTY_XLSX`.
- Лимит размера файла: 100 МБ (как у CSV).
